# 0003 — Descartar `dGTINCom` y `dCodProd` para el código de barra
...
Para el código de barra del artículo existen dos campos que parecerían adecuados:
...
No se usan `dGTINCom` ni `dCodProd` para el código de barra. Se transporta en
...
| `dCodProd` (C04) | Alfanumérico, 1–20 | Es el código interno del ítem, el número de material en SAP. Dato distinto del código de barra |
...
Reutilizarlo para el código de barra destruye la identificación interna del artículo,
...
| Emitir el código de barra en `dCodProd` y el SKU en `dInfEmFE` | Invierte el problema sin resolverlo, y contradice la semántica del campo oficial |
